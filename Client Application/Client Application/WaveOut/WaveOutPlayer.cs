...
namespace Client_Application.WaveOut
{
    ...
    public sealed class WaveOutPlayer : IDisposable
    {
        ...
        private bool _stopPlayLoop = false;
        ...
        public WaveOutPlayer(int device, int bufferSize, int bufferCount, BufferFillEventHandler fillProc)
        {
            ...
            _bufferDataHandle = GCHandle.Alloc(bufferData, GCHandleType.Pinned);
        ...
        {
            while (Interlocked.CompareExchange(ref _activeBuffers, 0, 0) != 0 && _playbackState != PlaybackState.Stopped)
            {
                Thread.Sleep(10);
            }
        }
