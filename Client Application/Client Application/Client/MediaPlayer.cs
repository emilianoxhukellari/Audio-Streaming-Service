    ...
    public sealed class MediaPlayer
    {
        ...
        private void StartSong() // Start playing sound.
        {
            if (_queue.Count > 0 && _dualSocket.Connected)
            {
                ...
                lock (_lock)
                {
                    ClientTCP.SendTCP(MOD1, 4, _dualSocket.MediaPlayerSocket);
                    ClientTCP.SendTCP(requestSongIdBytes, 4, _dualSocket.MediaPlayerSocket);
                }
                StartReceiveData();
        ...
        private void OptimizeStream(int start, int end) // Asks the server for the specific packets from start to end
        {
            lock (_lock)
            {
                ...
                TerminateReceive();
                _currentlyReceivingFlag.WaitOne();
                ClientTCP.SendTCP(MOD2, 4, _dualSocket.MediaPlayerSocket);
                ClientTCP.SendTCP(fromPacketBytes, 4, _dualSocket.MediaPlayerSocket);
                ClientTCP.SendTCP(toPacketBytes, 4, _dualSocket.MediaPlayerSocket);
                StartReceiveData();
            }
        }
        ...
        private void ReceiveFromServer()
        {
            ...
            while (true)
            {
                ...
                try
                {
                    mode = ClientTCP.ReceiveTCP(4, _dualSocket.MediaPlayerSocket);

                    if (Enumerable.SequenceEqual(mode, MOD1))
                    {
                        byte[] waveHeader = ClientTCP.ReceiveTCP(44, _dualSocket.MediaPlayerSocket);
                        _audioStream.Prepare(waveHeader);
                        _waveOutPlayer.Prepare(_audioStream.Format);
                        _preparedFlag.Set();
                    }

                    byte[] packetCountBytes = ClientTCP.ReceiveTCP(4, _dualSocket.MediaPlayerSocket);
                    ...
                    {
                        currentType = ClientTCP.ReceiveTCP(4, _dualSocket.MediaPlayerSocket);
                        if (Enumerable.SequenceEqual(currentType, DATA))
                        {
                            packet = ClientTCP.ReceiveTCP(4096, _dualSocket.MediaPlayerSocket);
                ...
                catch (SocketException) // Socket was disconnected; try to reconnect
                {
                    receivedAll = false;
